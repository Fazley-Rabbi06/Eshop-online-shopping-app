...
def add_to_cart(request,pk):
    ...
    print(f'Order item objects:{order_item}')
    print(order_item[0])
    order_qs = Order.objects.filter(user=request.user,ordered=False)
    print(f'oder qs:{order_qs}')
    if order_qs.exists():
        order = order_qs[0]
        print(f'If order exist:{order}')
        # print(order_qs[0])
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity > 1:
                order_item.quantity -=1
                order_item.save()
                messages.info(request, f"{item.name} item's quantity has been updated.")
                return redirect("App_Order:cart")
            else:
                order.orderitems.remove(order_item)
                order_item.delete()
